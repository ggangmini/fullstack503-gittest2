public class WhileEx {
    public static void main(String[] args) {
        ...
        // - 증강식을 반복 소스 코드가 있는 코드 블록 안에서 실행
        ...
        int j = 0;
        // 첫번째 while 문은 초기화 변수 값이 0으로 시작됨
        System.out.println("---첫번째 while 문 실행-----");
        while (j < 5) {
            System.out.println("j의 값:" + j);
            j++;
        }

        // 두번째 while문은 초기화 변수 j의 값이 이미 5이기 때문에 두번째 while문의 반복조건에 맞지 않음, 한번도 실행되지 않음
        //j=0; // while 문 사용 전 반드시 초기화 진행
        System.out.println("----------두번째 while 문 실행------------");
        while (j < 5) ;
        {
            System.out.println("두번째 while 문에서 j 의 값 : " + j);
            j++;
        }

        System.out.println("\n-------- for 2번 연속 시 주의-----\n");

        for (int k = 0; k < 5; k++) { // for문안에서 k 초기화하고 난후 출력하면 k가 사라져 밑에서 다시 사용가능
            System.out.println("첫번째 for문에서 k 의 값 :" + k);
        }
        for (int k = 0; k < 5; k++) {
            System.out.println("두번째 for문에서 k 의 값 :" + k);
        }
